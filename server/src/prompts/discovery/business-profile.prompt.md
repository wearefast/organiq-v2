...
- **social_media**: Scan every page footer, header, contact page, and about page for social media links. Extract platform name and full URL for every link found (LinkedIn, Twitter/X, Facebook, Instagram, YouTube, TikTok, GitHub, Pinterest, etc.). If footer content was not captured by the scraper (common with JavaScript-rendered footers) and you find fewer than 3 links, supplement with the brand's well-known official handles that you are confident about from general knowledge — mark these with a `"source": "inferred"` field. Only infer handles you are confident are official and current.
- **sitemap_urls**: Copy the `sitemapUrls` array from `pipeline_data.rawData.sitemapUrls` verbatim into this field.
- **analyst_notes**: Summarise coverage honestly — mention how many pages were scraped, key data sources used, and any confidence gaps.

...
This document is the strategist-workflow target state for Pulse, not a description of the current product implementation.

For the product rollout:

- English is the first implementation scope.
- Arabic should be added later on the same workflow/state architecture.
- The authenticated keyword workspace should implement this workflow.
- The public audit should remain a separate lightweight acquisition lane.
- Method 03 should remain a manual Ahrefs Content Gap import in the strategist workflow.
- Every checkpoint in this document should map to a persisted workflow artifact and internal approval record.
...
In Pulse, this same flow should be implemented as a dashboard workflow run with step-level status, artifact versioning, internal approvals, and final exports.
...
For Pulse implementation, the corresponding workflow-run inputs should be:

| Field | Description |
|-------|-------------|
| `projectId` | Keyword project container |
| `language` | `en` for the initial release |
| `country` | Market for SERP and Ahrefs calls |
| `websiteUrl` | Client domain |
| `seedKeywords` | Approved after Step 02 |
...
For Pulse implementation, Phase 2 should produce reviewed artifacts for Method 01, Method 02, Method 03, and consolidation before topical-map generation is allowed.
